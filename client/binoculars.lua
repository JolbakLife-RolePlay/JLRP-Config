...
local isBinocularsThreadActive = false
local shouldTurnHudBackOn
...
function RunBinocularsThread()
	if not isBinocularsThreadActive then
		isBinocularsThreadActive = true
	else
		return
	end
	CreateThread(function()
		local scaleform
		if Framework.UI.HUD.IsShowing() then
			shouldTurnHudBackOn = true
			Framework.UI.HUD.SetDisplay(0.0)
		end
		while binoculars do

			local lPed = Framework.PlayerData.ped
			...
			while binoculars and not IsEntityDead(lPed) and (GetVehiclePedIsIn(lPed) == vehicle) and IsPedUsingScenario(Framework.PlayerData.ped, "WORLD_HUMAN_BINOCULARS") and true do
		...
		SetSeethrough(false)
		isBinocularsThreadActive = false
		if shouldTurnHudBackOn then
...
function CheckInputRotation(cam, zoomvalue)
    ...
    if rightAxisX ~= 0.0 or rightAxisY ~= 0.0 then
        ...
        SetEntityHeading(Framework.PlayerData.ped,new_z)
    end
end
