...
CreateThread(function()
	...
	while true do
		...
		if IsInVehicle then
			local vehicle = GetVehiclePedIsUsing(Framework.PlayerData.ped)

			if vehicle > 0 then
				PlayerInVehicle = true
				PlayerVehicle = vehicle

				if GetPedInVehicleSeat(PlayerVehicle, -1) == Framework.PlayerData.ped then
					PlayerIsDriver = true
				else
		...
		if IsPedArmed(Framework.PlayerData.ped, 6) then
			wait = 0
			...
			if disable_reticle then		
				ManageReticle(Framework.PlayerData.ped)
			end

			-- Disable melee while aiming (may be not working)	
			--if IsPedShooting(Framework.PlayerData.ped) then	
...
CreateThread(function()
	...
	while not Framework.PlayerData.ped do Wait(1000) end
	while true do
		if IsPedArmed(Framework.PlayerData.ped, 6) then
			if IsPedShooting(Framework.PlayerData.ped) then
				--print(GetCurrentPedWeapon(ped))
				local _, wep = GetCurrentPedWeapon(Framework.PlayerData.ped)
				_, cAmmo = GetAmmoInClip(Framework.PlayerData.ped, wep)
				if recoils[wep] and recoils[wep] ~= 0 then
					...
					repeat 
						...
						if GetFollowVehicleCamViewMode() == 4 then
							if IsPedDoingDriveby(Framework.PlayerData.ped) then
